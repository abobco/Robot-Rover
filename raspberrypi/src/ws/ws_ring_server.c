...
int ws_create_thread(const char *domain, int port) {
	char *_domain; 
	if ( domain == NULL ){
		_domain = (char*)malloc(strlen(MY_DOMAIN) + 1);
		strcpy(_domain, MY_DOMAIN);
	}else {
		_domain = (char*)malloc(strlen(domain) + 1);
		strcpy(_domain, domain);
	}
	if ( port == 0 )
		port = 3000;

	struct ServerOptions s = { _domain, port };
	pthread_t thread_id;
...
xnet_vec3 get_motion_data(int idx ) {
