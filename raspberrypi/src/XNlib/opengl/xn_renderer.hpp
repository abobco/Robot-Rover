/*
    ...
    OpenGL review:
        ...
        Setup:
            ...
            4. Bind vertex data to vertex buffers. We now have 2 ints(VAO, VBO) to reference the
               data
            5. Set vertex attribute pointer. This is where the vertex array format must be given and
               saved
            5.b (optional)  repeat steps 3-5 w/ indices array, get EBO handle
            6. Compile shaders
        Loop:
            1. In any order:
                - Set shader uniforms
                - Bind vertex array
                - Activate shader
            2. call glDrawArrays(mode, first, count) or glDrawElements(mode, count, type, indices)
...
struct Mesh {
    ...
    std::pair<GLenum, GLenum> polygon_mode = std::pair(GL_FRONT_AND_BACK, GL_FILL);
    ...
    Mesh(const std::vector<glm::vec3> &verts, VertexFormat format = VERTEX_XYZ,
        ...
        // DUMP(verts.size());
        std::vector<float> v;
        v.insert(v.begin(), (float *)&verts.front(), (float *)&verts.front() + verts.size() * 3);
        DUMP(v.size());
    ...
    virtual void init(const std::vector<float> &verts, VertexFormat format = VERTEX_XYZ,
                      const std::vector<unsigned> &indices = std::vector<unsigned>()) {
        ...
        case VERTEX_XYZ:
            ...
            glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(float), &verts.front(),
                         GL_STATIC_DRAW);
    ...
    void setPolygonMode(GLenum face, GLenum mode) { polygon_mode = std::pair(face, mode); }
    ...
    virtual void draw() {
        glPolygonMode(polygon_mode.first, polygon_mode.second);
        glBindVertexArray(VAO);
        glDrawArrays(draw_mode, 0, verts.size() / stride);
        // switch (vert_format) {
        // case VERTEX_XYZ:
        //     if (draw_mode == GL_TRIANGLES)
        //         glDrawArrays(draw_mode, 0, verts.size() / 3);
        //     else
        //         glDrawArrays(draw_mode, 0, verts.size() / 3);
        //     break;
        // case VERTEX_XYZ_UV:
        ...
        //     // glPolygonMode(polygon_mode.first, polygon_mode.second);
        //     // glBindVertexArray(VAO);
        //     glDrawArrays(draw_mode, 0, verts.size() / 5);
        //     break;
        // default:
        //     break;
        // }
    ...
    static Mesh *gen_grid(float side_len = 1, int rows = 10, int cols = 10) {
        ...
        float halflen_rows = 0.5 * rows * side_len;
        float halflen_cols = 0.5 * cols * side_len;
    ...
    static Mesh *gen_box_batch(std::vector<glm::vec3> positions,
        ...
        const size_t cubesize = sizeof(gl::cube_verts) / sizeof(float);
        std::vector<float> verts;
        verts.reserve(cubesize * positions.size());

        for (const glm::vec3 &p : positions) {
            float new_verts[cubesize];
            std::copy(std::begin(gl::cube_verts), std::end(gl::cube_verts), std::begin(new_verts));
            for (unsigned i = 0; i < cubesize; i += 5) {
                new_verts[i] *= scale.x;
        ...
        Mesh *m = new Mesh(verts, xn::VERTEX_XYZ_UV);
...
struct BoxMesh : public Mesh {
    ...
    void init() {
        ...
        glBindVertexArray(VAO);
        glBindBuffer(GL_ARRAY_BUFFER, VBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(gl::cube_verts), gl::cube_verts, GL_STATIC_DRAW);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void *)0);
        glEnableVertexAttribArray(0);
    }
...
struct DrawNode {
    ...
    void transform(glm::vec3 translation, glm::vec3 scale = glm::vec3(1), float angle = 0.0f,
