...
struct Texture2D {
    ...
    Texture2D(const char *filename, int uniform_idx = GL_TEXTURE0, bool flip = true) {
    ...
    Texture2D(unsigned char *image_data, int len, int uniform_idx = GL_TEXTURE0, bool flip = true) {
        setTexParams(uniform_idx);
        stbi_set_flip_vertically_on_load(flip);
        unsigned char *data =
            stbi_load_from_memory(image_data, len, &width, &height, &channel_count, 0);
