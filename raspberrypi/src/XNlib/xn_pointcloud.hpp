...
public:
  ...
  static void hillclimb_transform(const PointCloud &a, PointCloud &b,
    ...
    auto rotate = [](glm::vec3 &a, float x) {
      a = glm::rotate(a, x, {0, 1, 0});
    };
  ...
  static void march_squares(const std::vector<PointCloud> &pcs, const int w,
    ...
    glm::vec3 cp(car_position.x - offset_out.x, 0,
                 car_position.y - offset_out.z);
    offset_out += 0.5 * s;
    offset_out.y = 0;
      ...
      for (int j = 0; j < h; j++) {
        if (spatial_hash(&cp, 1, i, 0, j, s).size() == 1) {
          ...
          time_spatial_hashing +=
              pio::time_diff_seconds(sp_start, pio::get_time());
        } else if (spatial_hash(&all_points.front(), all_points.size(), i, 0, j,
                                s)
