// sudo gdb OCV_Projects/build/arm_controller_main
...
struct ClawController {
    ...
    float max_diff = 0.12;
    std::vector<float> volt_buf = std::vector<float>();
};
...
void set_claw_target(ClawController &c, ClawTarget claw_target) {
    ...
    case CLAW_CLOSE: {
        if (c.volt_buf.size() == 0)
            break;
        while (fabsf(c.volt_buf.back() - c.rest_volts) < c.max_diff && cur_width < rest_width &&
               run) {
            cur_width += cur_step;
...
void arm_wait_action_complete(double timeout = 10) {
    ...
    time_sleep(0.25);
    while (time_diff_seconds(t1) < timeout) {
        bool finished = true;
        for (auto i = 0; i < armptr->ideal_chain.bone_count; i++) {
            for (const pio::SmoothServo &s : armptr->servos[i]) {
...
void arm_ctl_thread() {
    ...
    while (receiving_commands) {
        ...
        set_claw_target(claw_ctl, CLAW_OPEN);
        armptr->straighten();
        armptr->ideal_chain.print();
        armptr->ideal_chain.reset();
        arm_wait_action_complete();
        armptr->reset();
        // for (unsigned i = 0; i < armptr->ideal_chain.bone_count; i++) {
        //     armptr->positions[i] = armptr->ideal_chain.positions[i];
        // }
