...
void write_reg(unsigned char addr, unsigned char data, int CS) { bus_write(addr | 0x80, data, CS); }
...
void set_fifo_burst() { spiSendReceive(BURST_FIFO_READ); }

void flush_fifo(int CS) { write_reg(ARDUCHIP_FIFO, FIFO_CLEAR_MASK, CS); }

void start_capture(int CS) { write_reg(ARDUCHIP_FIFO, FIFO_START_MASK, CS); }

void clear_fifo_flag(int CS) { write_reg(ARDUCHIP_FIFO, FIFO_CLEAR_MASK, CS); }
