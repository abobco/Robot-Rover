...
//                    bone_start_rotations[i] = current.rotation;
            ...
            void resolve(vec3& target) {
                ...
                if ( vec3::dist_sqr(target, bones[0].position ) > chain_length*chain_length ) {
                    ...
                    for ( int i = 0; i < bone_count-2; i++) {
                        ...
                        // if ( vec3::dist_sqr(proj_mid, proj_pole) < ORTH_TOLERANCE  || vec3::dist_sqr(proj_mid*-1, proj_pole) < ORTH_TOLERANCE)
                        //     continue;
        ...
        struct IkConstraint {
           ...
           IkConstraint(int _num_axes, float _angle_min[], float _angle_max[], vec3 _axes[]) {
                num_axes = _num_axes;
        ...
        public:
            ...
            ServoChain(IkChain bonechain, std::vector<Servo> _servos[], std::vector<vec3> axes[]) {      
                ideal_chain = bonechain;

                servos = _servos;
//                servos = new std::vector<Servo>[bonechain.bone_count];

                // servos = new std::vector<pio::ServoAngular>[ideal_chain.bone_count];
                constraints = new IkConstraint[ideal_chain.bone_count];
...
//                        pio::ServoAngular s = _servos[i][j];
                        ...
                        // servos[i].push_back(pio::ServoAngular(s.ctl_pin, s.min_width, s.max_width));
//                        printf("loaded servo : %f %f\n", servos[i].at(j).servo.min_angle, servos[i].at(j).servo.max_angle);
...
//                ideal_chain.reset(pole);
                ...
                for ( int i = 0; i < ideal_chain.bone_count-1; i ++ ) {
                    for ( int j = 0; j < constraints[i].num_axes; j++ ) {
                        ...
                        // if ( vec3::dist_sqr(proj_pos, proj_target) < ORTH_TOLERANCE || vec3::dist_sqr(proj_pos*-1, proj_target) < ORTH_TOLERANCE )
                        //     continue;
                        ...
                        if ( constraints[i].num_axes>1){
                          s_ang = fmod(constraints[i].angles[j] + ang, constraints[i].angle_max[j]);
                          if ( s_ang < 0 ) {
                            s_ang =  constraints[i].angle_max[j] + s_ang;
                          }
                        }      
                        else 
                            s_ang = clamp(constraints[i].angles[j] + ang, constraints[i].angle_min[j], constraints[i].angle_max[j]);
                        ...
                        for ( int k = i+1;  k < ideal_chain.bone_count; k++ ) {
                            ...
                            for ( int l = 0; l < constraints[k].num_axes; l++ ) {
                                constraints[k].axes[l] = vec3::rotate_axis(constraints[k].axes[l], ax, ang);
                                constraints[k].axes[l].normalize();
                            }
                        }

                        for ( int k = j+1; k < constraints[i].num_axes; k++ ) {
                            constraints[i].axes[k] = vec3::rotate_axis(constraints[i].axes[k], ax, ang);
