...
json App::settings;
std::string App::settings_filepath;

std::string App::assetPath(const json& relpath){
	return (std::string)settings["asset_path"] + "/" + (std::string)relpath;
}

App::App(const std::string& settings_file){
...
void App::updateBoxBatch(std::vector<glm::vec3>& points, vulkan::Mesh& mesh, DrawNode& node, const glm::vec3& scale, const glm::vec4& color){
	if (points.size() == 0)
		return;
...
void App::updateUniforms(uint32_t imageIndex){
	ModelMatrix model;
	UniformLight light;
...
void App::update_path(GLFWwindow* window){
	glm::dvec2 cursor;
	glm::ivec2 winsize;
...
void App::window_pos_callback(GLFWwindow* window, int xpos, int ypos){
