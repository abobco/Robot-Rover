...
/*
    Cam distortion notes:

    - radial factor (barrel/fish eye effect)
        xdistorted=x(1+k1r^2+k2r^4+k3r^6)
        ydistorted=y(1+k1r^2+k2r^4+k3r^6)

    - tangential factor (b/c image lenses are not perfectly parallel to the
   imaging plane) xdistorted=x+[2p1xy+p2(r^2+2x^2)]
        ydistorted=y+[p1(r^2+2y^2)+2p2xy]

    - leads to 5 distortion params, represented by the 1x5 matrix:
        distortion_coefficients=(k1 k2 p1 p2 k3)

    - for unit conversion we use the formula:
           | x |    | fx  0  cx || X |
           | y |  = | 0   fy cy || Y |
           | w |    | 0   0  0  || Z |

            where w = Z ( homogrophy coordinate system )
             fx, fy = unknown camera focal lengths
             cx, cy = unknown optical centers in pixel coords
...
public:
  ...
  void readImageThread() {
    ...
    while (connected) {
      ...
      //   char buffer[65536 * 8] = {0};
      //   char *buf;

      // read file size in bytes
      //   uint32_t tmp, left;
      //   buf = (char *)&tmp;
      //   xn::read_buf(client_sock, buf, sizeof(tmp), 0);
      //   left = tmp;
      //   if (verbose)
      //     printf("%d byte jpeg\n", left);
      ...
      //   buf = buffer;
      //   xn::read_buf(client_sock, buf, left);

      //   // convert jpg byte array into opencv mat
      //   cv::Mat rawData(1, sizeof(buffer), CV_8SC1, (void *)buffer);

      uint32_t pic_buflen;
      read_int<uint32_t>(client_sock, pic_buflen, 0);
      //   frame_mut.lock();
      //   delete pic_buffer;
      char *pic_buffer = new char[pic_buflen];
      read_buf(client_sock, pic_buffer, pic_buflen, 0);
      cv::Mat rawData(1, pic_buflen, CV_8SC1, (void *)pic_buffer);
...
public:
  ...
  Mat nextImage() {
    ...
    Mat view0;
    // inputCapture >> view0;
    view0 = inputStream.getLastFrame();
    view0.copyTo(result);
    /*           }
...
//! [run_and_save]
int calibrate_camera(String inputSettingsFile, int winSize = 11) {
  //! [file_read]
  Settings s;
  ...
  // s.inputStream = xnnet::VideoStream(8080, true);
  xn::VideoStream istrm(5002, true);
  ...
  for (;;) {
    ...
    if (found) // If done with success,
    {
      ...
      if (mode ==
          ...
          (!s.inputStream.isOpened() ||
           clock() - prevTimestamp > s.delay * 1e-3 * CLOCKS_PER_SEC)) {
        imagePoints.push_back(pointBuf);
        prevTimestamp = clock();
        blinkOutput = s.inputStream.isOpened();
      }
    ...
    char key = (char)waitKey(s.inputStream.isOpened() ? 50 : s.delay);
    ...
    if (s.inputStream.isOpened() && key == 'g') {
      mode = CAPTURING;
      imagePoints.clear();
    }
    //! [await_input]
