...
private:
  ...
  void createScene() {
    ...
    { // add gridbox batch drawnode
      ...
      scene.addChild("color_lit",
                     vulkan::Mesh::gen_box_batch(ctx, gridverts,
    ...
    for (int i = 0; i < getArm()->numJoints; i++)
      scene.drawMap["arm"]->addChild(
          new DrawNode(&gp, &cubeMesh),
          PushConstColor(getArm()->positions[i].toGLM(), glm::vec3(0.1f),
                         vulkan::sample_colors[2]));
  ...
  ImDrawData *drawImgui() {
    ...
    bool t = true;
    console->Draw("Ssh: Console", &t, *robot);
  ...
  void updateUniforms(uint32_t imageIndex) {
    ...
    for (auto i = 0; i < getArm()->numJoints; i++) {
      model = transform_model(car_pos_3d);
      model = rotate_eulers(glm::vec3(0, car_ang_corrected, 0), model);
      model = transform_model(getArm()->positions[i].toGLM(), glm::vec3(0.1f),
                              model);
  ...
  void createArmMesh() {
    ...
    for (auto i = 0; i < n; i++) {
      ...
      model = rotate_eulers(glm::vec3(0, car_ang_corrected, 0), model);
      model =
          transform_model(getArm()->positions[i].toGLM(), glm::vec3(1), model);
