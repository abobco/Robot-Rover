...
RobotArm::RobotArm() {
    ...
    joints = ik::ServoChain(arm, servos, &run, 172);
...
void RobotArm::updateJoints(double distance) { joints.grab_safe(distance, target); }
...
void RobotArm::setClawState(ClawTarget state) {
    ...
    case CLAW_CLOSE: {
        if (claw_ctl.volt_buf.size() == 0)
            break;
        while (fabsf(claw_ctl.volt_buf.back() - claw_ctl.rest_volts) < claw_ctl.max_diff &&
