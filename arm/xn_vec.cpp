...
float vec3::dot(const vec3 &l, const vec3 &r) { return l.x * r.x + l.y * r.y + l.z * r.z; }
...
vec3 vec3::operator+(const vec3 &r) { return {this->x + r.x, this->y + r.y, this->z + r.z}; }

vec3 vec3::operator-(const vec3 &r) { return {this->x - r.x, this->y - r.y, this->z - r.z}; }

vec3 vec3::operator*(const vec3 &r) { return {this->x * r.x, this->y * r.y, this->z * r.z}; }

vec3 vec3::operator*(const float &r) { return {this->x * r, this->y * r, this->z * r}; }
...
void nanosleep(uint64_t ns) { std::this_thread::sleep_for(std::chrono::nanoseconds(ns)); }
