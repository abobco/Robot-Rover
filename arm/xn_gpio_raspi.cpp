...
int PigDaemon::i2cClose(unsigned handle) {
  return error_check(i2c_close(pi, handle));
}
...
int PigDaemon::serialDataAvailable(int h) {
  return error_check(serial_data_available(pi, h));
}
...
RotaryEncoder::RotaryEncoder(unsigned pin_a, unsigned pin_b,
                             void (*callback)(int), int mode, int step)
...
void RotaryEncoder::callback_internal(int gpio, int level, uint32_t tick,
                                      void *userdata) {
...
Stepper::Stepper(int pin_dir, int pin_step, float dutycycle, unsigned freq,
