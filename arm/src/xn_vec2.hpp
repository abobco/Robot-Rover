...
namespace xn {
template <class numtype> numtype clamp(numtype n, numtype minval, numtype maxval) {
...
struct vec3 {
    ...
    static inline float dist(const vec3 &l, const vec3 &r) { return sqrt(dist_sqr(l, r)); }
    ...
    inline vec3 abs() { return {fabsf(x), fabsf(y), fabsf(z)}; }

    inline vec3 operator+(const vec3 &r) { return {this->x + r.x, this->y + r.y, this->z + r.z}; }

    inline vec3 operator-(const vec3 &r) { return {this->x - r.x, this->y - r.y, this->z - r.z}; }

    inline vec3 operator*(const vec3 &r) { return {this->x * r.x, this->y * r.y, this->z * r.z}; }

    inline vec3 operator*(const float &r) { return {this->x * r, this->y * r, this->z * r}; }
