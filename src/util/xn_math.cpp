...
float expected_val(float v[], int num_points, float p[]) {
  float eval = 0;
  if (p == NULL) {
...
float covariance(float v1[], float v2[], int num_vars) {
  ...
  float *vn1 = new float[num_vars];
  float *vn2 = new float[num_vars];
  ...
  float e = expected_val(vn1, num_vars);
  delete vn1;
  delete vn2;
  return e;
}

float **cov_mat(float **df, int rows, int cols, bool fill_redundant_values) {
...
glm::vec3 eigenval_3x3(glm::mat3 a) {
  ...
  if (p1 == 0) {
    ...
    eigv[2] = q + 2 * p * cos(phi + 2 * (float)M_PI / 3);
    eigv[1] =
        3 * q - eigv[0] - eigv[2]; // b/c trace(a) = eig[0] + eig[1] + eig[2]
  }

  return eigv;
}

glm::vec3 find_eigvec(glm::mat3 &eigmat, bool print_info) {
  glm::vec3 out(0);
  for (int j = 0; j < 3; j++)
    for (int k = 0; k < 3; k++)
...
std::vector<Eigen> eigenvec_3x3(float **a) {
  ...
  if (mult) {
    ...
    out.push_back(
        {1, glm::cross(eigvec[0],
                       eigvec[1])}); // get 3rd vector thats perpendicular
...
std::vector<glm::vec3> spatial_hash(glm::vec3 pt[], int n, int x, int y, int z,
  ...
  for (int i = 0; i < n; i++) {
    if (pt[i].x >= bmin.x && pt[i].x <= bmax.x && pt[i].y >= bmin.y &&
        pt[i].y <= bmax.y && pt[i].z >= bmin.z && pt[i].z <= bmax.z)
      out.push_back(pt[i]);
  }
...
OBB pca_obb(std::vector<glm::vec3> pt, bool print_info) {
  OBB out;
  out.pos = glm::vec3(0);

  // exit early to avoid front() calls on empty vector
  if (pt.size() == 0) {
    for (int i = 0; i < 3; i++) {
  ...
  // subtract means
  for (i = 0; i < 3; i++) {
    float m = expected_val(df[i], (int)pt.size());
    for (j = 0; j < pt.size(); j++)
      df[i][j] -= m;
  }

  // covariance matrix
  float **cm = cov_mat(df, (int)pt.size(), 3);
  for (i = 0; i < 3; i++)
    delete[] df[i];
  delete[] df;
  ...
  for (i = 0; i < 3; i++) {
    ExtremeInfo extrema;
    // already checked that this vector isn't empty at start of function
    extreme_pts_in_dir(eigenvectors[i].vec, &pt.front(), (int)pt.size(),
                       extrema);
...
void sort_rays_by_angle(const std::vector<glm::vec3> &in_vec,
                        std::vector<glm::vec3> &out_vec, glm::vec3 normal) {
  std::vector<glm::vec3> out;

  // project onto ground plane
  project_points(in_vec, out, glm::vec3(0), normal);
...
void scan_split_lines(const std_vec2d<glm::vec3> &samples,
  ...
  for (its = samples_out.begin(); its < samples_out.end(); its++) {
    for (its2 = its + 1; its2 < samples_out.end(); its2++) {
      ...
      distance_sqr = closest_pt_segment_segment(
          its->front(), its->back(), its2->front(), its2->back(), s, t, c1, c2);
...
int test_separating_axis(const std::vector<glm::vec2> &a,
  ...
  extreme_pts_in_dir(axis, &a.front(), (int)a.size(), r[0]);
  extreme_pts_in_dir(axis, &b.front(), (int)b.size(), r[1]);
