...
namespace xn {
namespace ik {
IkChain::IkChain(int _bone_count, vec3 ordered_bones[], vec3 &_pole,
                 int _iterations) {
...
void IkChain::reset() {
  vec3 pole = {0, 0, 1};
  vec3 bonechain[] = {vec3{0, 0, 0}, vec3{0, 1, 0},
                      vec3{0, 1 + 6.9f / 17.2f, 0},
  ...
  for (int i = 0; i < ideal_chain.bone_count - 1; i++) {
    for (int j = 0; j < servos[i].size(); j++) {
      ...
      if (servos[i].size() > 1 && j == 0)
        s_ang =
            fmod(servos[i][j].target_angle + ang, servos[i][j].servo.max_angle);
      else
        s_ang = clamp<float>(servos[i][j].target_angle + ang,
                             servos[i][j].servo.min_angle,
